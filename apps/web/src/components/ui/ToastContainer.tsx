    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          {toast.title && (
            <h3 className={`text-sm font-medium ${style.title}`}>{toast.title}</h3>
          )}
          <p className={`text-xs ${toast.title ? 'mt-1' : ''} ${style.message}`}>
            {toast.message}
          </p>
        </div>
        <button onClick={onClose} className={style.close} aria-label="Close">
