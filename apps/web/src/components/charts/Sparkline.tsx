...
export function Sparkline({ values, width = 240, height = 48, stroke = '#2a6dff', fill = 'rgba(42,109,255,0.15)' }: SparklineProps) {
  if (values.length === 0) return null
  const max = Math.max(...values)
  const min = Math.min(...values)
  const dx = width / (values.length - 1 || 1)
  const normalize = (v: number) => height - ((v - min) / (max - min || 1)) * (height - 4) - 2
