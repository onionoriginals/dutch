...
export const schedulePointSchema = z.object({ t: z.number().nonnegative(), price: z.number().nonnegative() })

export const scheduleSchema = z.object({ points: z.array(schedulePointSchema).min(2) })

export const feeSchema = z.object({ name: z.string(), amount: z.number().nonnegative(), type: z.enum(['flat', 'percent']).default('flat') })
...
export function parseAuctionFromQuery(search: URLSearchParams): Auction | null {
  const payload = search.get('state') ?? search.get('s')
